...
def worldMap(game):
    ...
    while True:
        ...
        destination = displayMap(game)

        if destination:
            action = displayTravelMenu(game, destination)
...
def displayMap(game):
    ...
    for i in game_data.HUBLIST:
        if i.coords == game.coords:
            ...
            nameSurf = cfg.AR15.render(game.player.name, True, cfg.BLACK)
    ...
    if not playerDrawn:
        pygame.draw.circle(cfg.DISPLAYSURF, cfg.BLACK, game.coords, 15, 2)
        pygame.draw.circle(cfg.DISPLAYSURF, cfg.GREEN, game.coords, 8)
        nameSurf = cfg.AR15.render(player.name, True, cfg.BLACK)
        nameRect = nameSurf.get_rect()
        nameRect.midbottom = (game.coords[0], game.coords[1] - 12)
        cfg.DISPLAYSURF.blit(nameSurf, nameRect)

def displayTravelMenu(game, destination):
    ...
    while True:
        ...
        displayMapBackground(game, destination)
        action = displayConfirmTravelMenu(game, destination)
...
def displayMapBackground(game, destination):
...
def displayConfirmTravelMenu(game, destination):
    ...
    distance = common_functions.getDistance(game.coords, destination.coords) #gets the distance between current location and destination
